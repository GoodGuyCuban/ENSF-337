...
int search(const Point* struct_array, const char* label, int n)
{
    ...
    // search for the label in the array of struct Point without using any string library function
    // return the index of the element in the array if the label is found
    // return -1 if the label is not found
    int i;
    for(i=0; i<n; i++)
    {
        if(struct_array[i].label[0] == label[0] && struct_array[i].label[1] == label[1])
            return i;
    }
    return -1;
}
...
void reverse (Point *a, int n)
{
    ...
    // reverse the order of the elements in the array of struct Point
