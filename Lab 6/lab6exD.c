...
void String_append(String *dest, const String* source){
    ...
    if(source -> length > 0) {
        if(dest -> length == 0) {
            ...
            char *temp = malloc(dest -> length + source -> length + 1);
            if(temp == NULL) {
                printf("malloc failed: Memory was unavailable...\n");
                exit(1);
            }
            strcpy(temp, dest -> dynamic_storage);
            strcat(temp, source -> dynamic_storage);
            free(dest -> dynamic_storage);
            dest -> dynamic_storage = temp;
...
void String_truncate(String *dest, int new_length){
    ...
    if(new_length < 0) {
        printf("new_length must be greater than or equal to 0\n");
        exit(1);
    }

    if(new_length == 0) {
        create_empty_string(dest);
    } else if(new_length < dest -> length) {
